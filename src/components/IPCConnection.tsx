    ...
    <>
      ...
      <svg className="absolute top-0 left-0 w-full h-full pointer-events-none">
        ...
        {dataTransfers.map(transfer => {
          ...
          const posX = fromPosition.x + (toPosition.x - fromPosition.x) * progress;
          const posY = fromPosition.y + (toPosition.y - fromPosition.y) * progress;
        ...
        {connection.type === 'queue' && connection.capacity && (
          <text
            ...
            textAnchor="middle"
            className={`text-xs ${
              connection.currentLoad && connection.currentLoad > connection.capacity * 0.8
                ? 'fill-amber-500'
