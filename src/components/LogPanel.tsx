...
const LogPanel: React.FC<LogPanelProps> = ({ logs, onClearLogs }) => {
  ...
  const getLogStyle = (
    ...
    const baseStyle = {
      info: "border-blue-200 bg-blue-50 dark:border-blue-900 dark:bg-blue-950/50  text-blue-700",
      warning:
        "border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/50 text-amber-700 ",
      error: "border-destructive/50 bg-destructive/10",
      success:
        "border-green-200 bg-green-50 dark:border-green-900 dark:bg-green-950/50 text-green-700",
    };
          ...
          <div className="flex flex-col gap-2">
            {filteredLogs.map((log) => (
              <Collapsible key={log.id} className="w-full">
                <Alert
                  className={`py-2 px-3 ${getLogStyle(
                    log.type,
                ...
                >
                  <div className="flex items-start gap-2">
                    <div className="mt-0.5">
                      {getLogIcon(log.type, log?.additionalInfo?.severity)}
                    </div>
                ...
                {log.additionalInfo && (
                  <CollapsibleContent className="bg-muted/50 rounded-b-md p-3 -mt-1 text-xs border-x border-b">
                    <div className="space-y-2">
                        ...
                        <div>
                          ...
                          <span
                            className={`ml-2 px-2 py-0.5 rounded ${
                              log.additionalInfo.severity === "high"
                                ? "bg-destructive/20 text-destructive"
                                : log.additionalInfo.severity === "medium"
                      ...
                      {log.additionalInfo.processes &&
                        log.additionalInfo.processes.length > 0 && (
                          <div>
                            <span className="font-semibold">
                              Affected Processes:
                            </span>
                            <ul className="list-disc ml-5 mt-1">
                              {log.additionalInfo.processes.map(
                                (process, idx) => (
                      ...
                      {log.additionalInfo.resources &&
                        log.additionalInfo.resources.length > 0 && (
                          <div>
                            <span className="font-semibold">
                              Resources Involved:
                            </span>
                            <ul className="list-disc ml-5 mt-1">
                              {log.additionalInfo.resources.map(
                                (resource, idx) => (
                      ...
                      {log.additionalInfo.solutions &&
                        log.additionalInfo.solutions.length > 0 && (
                          <div>
                            <span className="font-semibold">
                              Possible Solutions:
                            </span>
                            <ul className="list-disc ml-5 mt-1">
                              {log.additionalInfo.solutions.map(
                                (solution, idx) => (
